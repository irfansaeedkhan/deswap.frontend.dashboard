...
const swapRequests = Array.from({ length: 12 }, (_, i) => {
  const matic = Number((120 + i * 15.5).toFixed(2));
  const daw = Number((matic * 0.82).toFixed(2));
  const wallet = `0xUser${String((i % 8) + 1).padStart(4, "0")}6634C0532925a3b844Bc9e7595`;
  return {
    _id: `swap-${i + 1}`,
    created_at: new Date(Date.now() - i * 86400000 * 2).toISOString(),
    IncommingTxHash: `0xin${String(i + 1).padStart(4, "0")}aabbccddeeff00112233445566778899`,
    OutgoingTxHash: `0xout${String(i + 1).padStart(4, "0")}aabbccddeeff00112233445566778899`,
    amountInMatic: matic,
    amountInDeswap: daw,
    correctAmountInDeswap: Number((daw - 1.25).toFixed(2)),
    TransactionValid: i % 5 !== 0,
    transactionIssueDescription:
      i % 5 === 0 ? "Incoming amount does not match the quoted MATIC" : "",
    Status: ["Requested", "Active", "Pending"][i % 3],
    UserTo: {
      emailid: `user${(i % 8) + 1}@deswap.co`,
...
const claimedNetworkAdmin = Array.from({ length: 16 }, (_, i) => ({
  ...
  ValidRequest: i % 4 === 0 ? "Invalid" : "Valid",
  RejectReason: i % 4 === 0 ? "Amount mismatch" : "None",
