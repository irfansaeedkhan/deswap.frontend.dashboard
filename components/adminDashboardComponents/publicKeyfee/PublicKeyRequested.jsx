...
function PublickeyFeeRequested() {
  ...
  const updateSuccesFullStatus = async (updateData) => {
    try {
      ...
      let updateResult = await axios.post(
        `${process.env.NEXT_PUBLIC_PLATFORM_URL}/api/admin/publickey/update/details`,
        { data: encryptionData },
        {
  ...
  const fetchCompanyFee = async (data) => {
    try {
      ...
      setPublickeyFeeTable1List(<TableLoader colSpan={13} />);
      ...
      let result = await axios.post(
        `${process.env.NEXT_PUBLIC_PLATFORM_URL}/api/admin/publickey/fetch/requested`,
        { data: encryptionData },
        {
          withCredentials: true,
          headers: {
...
/**/
export default connect(mapStateToProps, mapDispatchToProps)(PublickeyFeeRequested);
