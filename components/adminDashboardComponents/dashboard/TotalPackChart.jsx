...
const TotalPackChart = (props) => {

  const months=['Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec'];
  const [users,setUsers]=useState();
  const [labelss,setLabels]=useState([0])
  const [data,setData]=useState({labels:"",datasets: [{
    label: "Total Value of All Packs Sold",
    fill: false,
    borderColor: '#0494E8',
    data: "",
  }
      ...
      <span className="">
        <span className="row">
            <span className="col-9">
                  <p className="CardContainer">Total Value of All Packs Sold</p>
                  </span>
                  <span className="col-3 text-right">
            <MonthDropdown onSelectIndex={(index)=>{fetchTotalPacks(index)}} />
        </span>
        </span>
        <br/>
        <div
