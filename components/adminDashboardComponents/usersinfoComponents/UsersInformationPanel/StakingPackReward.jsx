import React from "react";
import SmRightArrow from "@/assets/svgAssets/SmRightArrow";
import SmLeftArrow from "@/assets/svgAssets/SmLeftArrow";

function StakingPackReward() {
  return (
    <div className="tableContainer">
    <p>Stacking Pack Reward</p>
    <div className="tableContainerTable customScroll">
      <table cellPadding="0" cellSpacing="0" border="0">
        <thead>
          <tr>
            <th>Total Stacking Pack Rewards</th>
            <th>Total Available Stacking Pack Rewards</th>
            <th>Total Claimed Stacking Pack Rewards</th>
          </tr>
        </thead>
        <tbody>
          <tr>
            <td>0</td>
            <td><button className="locked">Pending</button></td>
            <td>0</td>
          </tr>
          <tr>
            <td>1</td>
            <td><button className="Unlocked">approved</button></td>
            <td>2</td>
          </tr>
        </tbody>
      </table>
    </div>
    <div className="pagination">
      <div className="total">
        <p>Total Items :  21</p>
        {"  "}
      </div>

      <div className="content_detail__pagination cdp" actpage="1">
        <a href="#" className="cdp_i">
          <SmLeftArrow />
        </a>
        <a href="#" className="cdp_i">
          1
        </a>
        <a href="#" className="cdp_i">
          2
        </a>
        <a href="#" className="cdp_i">
          <SmRightArrow />
        </a>
      </div>
    </div>
    <div className="pagination mobile">
      <div className="total">
        <p>Total Items :  221</p>
        {"  "}
      </div>

      <div className="content_detail__pagination cdp" actpage="1">
        <a href="#" className="cdp_i">
          <SmLeftArrow />
        </a>
        <a href="#" className="cdp_i">
          1
        </a>
        <a href="#" className="cdp_i">
          2
        </a>
        <a href="#" className="cdp_i">
          <SmRightArrow />
        </a>
      </div>
    </div>
  </div>
  )
}

export default StakingPackReward