...
const PacksPurchasedGraph = (props) => {

  const months=['Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec'];
  const [users,setUsers]=useState();
  const [labelss,setLabels]=useState([0])
  const [data,setData]=useState({labels:"",datasets: [{
    label: "Total Value of Purchased Pack",
    fill: false,
    borderColor: '#0494E8',
    data: "",
  }
  ...
  const fetchTotalRewards=async(month)=> {
    ...
    try {
        ...
        let result = await axios.post(
            `${process.env.NEXT_PUBLIC_PLATFORM_URL}/api/admin/purchasedpack/fetch/graph`,
            { data:encryptionData},
      ...
      <span className="">
        <span className="row">
            <span className="col-9">
                  <p className="CardContainer">Total Value of Purchased Pack</p>
                  </span>
...
export default PacksPurchasedGraph;
