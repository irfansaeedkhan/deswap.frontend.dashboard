...
/**
 * @api {post} /api/users/purchasedpack/fetch/claimmed/ Fetch Purchased Pack claimmed
 * @apiName FetchPurchasedPackclaimmed
 * @apiGroup User/Pack
 * @apiVersion 1.0.0
 * @apiDescription Fetch Purchased Pack claimmed
 * @apiBody {Number} [skip=0]
 * @apiBody {Number} [limit=10]
 * @apiExample {axios} Example usage:
 * axios.post('/api/users/purchasedpack/fetch/claimmed/', {
 * skip: 0,
 * limit: 10
 * })
...
const handler = async(req, res) => {
    try {
        ...
        let totalData = await purchasedpack.find({
        ...
        }).skip(req.body.skip).limit(req.body.limit).populate("PackID ClaimmedID");

        res.setHeader('response-security', true)
        res.status(200).json({ data: await responseBodyEncryption({ data: result, error: null, totaldata: totalData }), type: "userauth" });

    } catch (e) {
        console.log(e);
        res.status(400).json({ data: null, error: "Failed to fetch", totaldata: 0 });
    }
}
