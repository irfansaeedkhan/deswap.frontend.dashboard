...
/**
 * @api {post} /api/users/purchasedpack/fetch/all/ Fetch Purchased Pack all
 * @apiName FetchPurchasedPackall
 * @apiGroup User/Pack
 * @apiVersion 1.0.0
 * @apiDescription Fetch Purchased Pack all
 * @apiBody {Number} [skip=0]
 * @apiBody {Number} [limit=10]
 * @apiExample {axios} Example usage:
 * axios.post('/api/users/purchasedpack/fetch/all/', {
 * skip: 0,
 * limit: 10
 * })
...
const handler = async(req, res) => {
    try {
        ...
        let result = await purchasedpack.find({
            ...
            Status: { $in: ["Active", "Claimmed","Inprogress", "Requested"] }
        }, {}, {
            sort: {
                "created_at": "asc"
            }
