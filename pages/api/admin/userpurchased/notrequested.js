...
const handler = async(req, res) => {
    try {
        ...
        //status not equal to requested
        let totalPacks = await userRequested.find({ Status: { $ne: "Requested" } }).countDocuments();
        let Packs = await userRequested.find({ Status: { $ne: "Requested" } }).populate({ path: "UserID", select: 'emailid walletaddress' }).populate({ path: "NftLicense" }).skip(offset).limit(limit);
