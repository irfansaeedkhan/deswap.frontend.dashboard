...
const handler = async(req, res) => {
    try{
        ...
        let totalPacks = await PurchasedClammingPack.find({Status: { $in: ["Inprogress","Active","Claimmed","Deactive","Rejected"]}}).countDocuments();
        
        let Packs = await PurchasedClammingPack.find({
            Status: { $in: ["Inprogress","Active","Claimmed","Deactive","Rejected"]}
