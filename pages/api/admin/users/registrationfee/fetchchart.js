...
const handler = async (req, res) => {
    try{
        ...
        let registerFeeValidator = await Joi.object({
            startDate: Joi.date().optional(),
            lastDate: Joi.date().optional()
          });
      
          const { error, value } = registerFeeValidator.validate(req.body, options);
        ...
        await getMongoDBConnection();
          let result= await UserRegistrationFees.aggregate(
            [
                {
                    $match: {
