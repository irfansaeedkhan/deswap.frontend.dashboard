...
const handler = async(req, res) => {
    try {
        ...
        //get all users fee
        const regFeeList = await regFee.find({}).populate({path:"uuid",select:"emailid walletaddress"}).skip(offset).limit(limit).sort({ createdAt: sort });
        if (!regFeeList) {
            res.status(400).json({ data: null, error: "Invalid regFeeList" });
            return;
        }

        const total = await regFee.count();
