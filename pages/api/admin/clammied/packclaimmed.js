...
/**
 * @api {post} /api/admin/clammied/packclaimmed Get Pack Claimmed
 * @apiName Get Pack Claimmed
 * @apiGroup Admin/Clammied
 * @apiVersion 1.0.0
 * @apiDescription Get Pack Claimmed
 * @apiBody {Number} [limit] Limit
 * @apiBody {Number} [offset] offset
 * @apiExample {axios} Example usage:
 * axios.post('/api/admin/clammied/packclaimmed',{
 *   limit:10,
 * offset:0
 * });
 * @apiSuccess {String} data Data of the pack claimmed
 * 
 */
const handler = async(req, res) => {
    try {
        ...
        let totalPacks = await PackClaimmed.find({Status: { $in: ["Rejected","Deactive","Claimmed"]}}).countDocuments();
        let Packs = await PackClaimmed.find({
            Status: { $in: ["Rejected","Deactive", "Claimmed"]}
