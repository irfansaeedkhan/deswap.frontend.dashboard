...
import {web3DeSwapContract} from "../wallet/index"
import ejs from "ejs";
...
const startingTransactionEmail = async(emaildata)=>{
  try{
      ...
      let result = await sendEmail({
          ...
          bodyText: "Deswap starting transaction for public key : "+emaildata.publickey+" amount : "+emaildata.amount+" DAW",
...
const successTransactionEmail = async(emaildata)=>{
  try{
      ...
      let result = await sendEmail({
          ...
          bodyText: "Deswap transaction success for public key : "+emaildata.publickey+" amount : "+emaildata.amount+" DAW , Transactionhash : "+emaildata.txhash,
          bodyHTML: htmltempate,
      });
  }catch(e){
      console.log("!!!! Failed to send email",emaildata)
...
const failedTransactionEmail = async(emaildata)=>{
  try{
      ...
      let result = await sendEmail({
          ...
          bodyText: "Deswap transaction failed for public key : "+emaildata.publickey+" amount : "+emaildata.amount+" DAW",
          bodyHTML: htmltempate,
      });
  }catch(e){
      console.log("!!!! Failed to send email",emaildata)
...
const fetchDeswapBalance = async (contractweb) =>{
    try{
        ...
        //console.log("Public key : ",process.env.ADMIN_POLYGON_PUBLIC_KEY)
...
module.exports.sendTransaction = async (data)=>{
    try{
        ...
        let accountBalanceUser = await web3.utils.fromWei(adminAccountBalance);

        let contract = await web3DeSwapContract(web3);
        
        let deswapBalance = await fetchDeswapBalance(contract)
        
        if(Number(deswapBalance)<Number(data.amount)){
        ...
        /*
        //Checking wether admin have account balance to transfer to user account
        if(accountBalanceUser<data.amount){

            //Return response  and also inform admin email id
        ...
        let count = await  web3.eth.getTransactionCount(process.env.ADMIN_POLYGON_PUBLIC_KEY);
